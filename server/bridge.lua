...
function Bridge.removeMoney(source, account, amount, reason)
    ...
    if framework == 'qb' or framework == 'qbx' then
        ...
        return p.Functions.RemoveMoney(account, amount, reason or 'as-birthcert') == true
    elseif framework == 'esx' and ESX then
        local x = ESX.GetPlayerFromId(source)
...
function Bridge.addMoney(source, account, amount, reason)
    ...
    if framework == 'qb' or framework == 'qbx' then
        ...
        p.Functions.AddMoney(account, amount, reason or 'as-birthcert-refund')
        return true
    elseif framework == 'esx' and ESX then
        local x = ESX.GetPlayerFromId(source)
        if not x then return false end
...
--- True when the player is carrying a birth certificate item with this number. If the inventory can't
--- be searched it says yes, so a certificate is never refused because of a bridge problem.
function Bridge.holdsCertificate(source, number)
    if inventory == 'ox_inventory' then
...
function Bridge.sendPhoneMail(source, identifier, from, subject, body)
    local ok, err = pcall(function()
        ...
        if not email then
            print(('^5[as-birthcert]^0 mail not sent: %s has no email account in the Mail app'):format(tostring(identifier)))
            return
        end
        ...
        print(('^5[as-birthcert]^0 mail to %s was not delivered by sd-phone'):format(email))
    end)
    if not ok then print(('^5[as-birthcert]^0 mail failed: %s'):format(tostring(err))) end
end
